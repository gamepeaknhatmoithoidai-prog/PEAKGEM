...
export class Chapter2Scene extends Phaser.Scene {
  ...
  private buildWorld(): void {
    ...
    if (this.textures.exists('bg-ch2')) {
      // bg-ch2.jpg is 736×414 — scale tile to fill 540 px game height exactly
    ...
    // ── Mid-level platforms — invisible physics blocks only ───────────
    ...
    // ── World objects ─────────────────────────────────────────────────
    // Hut at start — visual only, NO physics block (invisible block would push
    // the player up 60px off the ground when they walk into the hut area).
    if (this.textures.exists('hut'))
      this.add.image(400, GROUND_Y - 35, 'hut').setScale(1.2).setDepth(DEPTH_WORLD + 1);

    // Heavy machinery silhouette — drawn with graphics (no missing texture),
    // NO physics block (same reason: avoids player riding invisible box).
  ...
  private buildNPCs(): void {
    ...
    // 3×3 sheets (lan, thang): frame 682×682 px, scale 0.15 → y = GROUND_Y - 46.
    // 4×2 sheet  (hung):       frame 512×1024 px, scale 0.11 → y = GROUND_Y - 51.
    const defs = [
      { key: 'npc-lan',   x: 360,  y: GROUND_Y - 46, name: 'Lan',              dialog: 'ch2-lan',   scale: 0.15 },
      { key: 'npc-hung',  x: 1400, y: GROUND_Y - 51, name: 'Hùng',             dialog: 'ch2-hung',  scale: 0.11 },
      { key: 'npc-thang', x: 2050, y: GROUND_Y - 46, name: 'Nguyễn Văn Thắng', dialog: 'ch2-thang', scale: 0.15 },
    ];
    for (const d of defs) {
  ...
  private buildEvidenceZones(): void {
    ...
    for (const p of positions) {
      ...
      const sprite = this.textures.exists('stake')
        ? this.add.image(p.x, p.y, 'stake').setScale(1.4).setDepth(DEPTH_WORLD)
        : null;
  ...
  private buildCameraUI(): void {
    ...
    this.cameraIcon = this.textures.exists('camera-item')
      ? this.add.image(this.player.x + 18, this.player.y - 24, 'camera-item')
          .setScale(1.3).setDepth(DEPTH_WORLD + 3).setVisible(false).setScrollFactor(1)
      : this.add.image(0, 0, '__DEFAULT').setDisplaySize(1, 1).setVisible(false);
  }
  ...
  update(_t: number, delta: number): void {
    ...
    // Hint
    let hint = '';
    if (!this.cameraMode && nearNPC && !nearNPC.isDone) hint = `💬 E — Nói chuyện với ${nearNPC.npcName}`;
  ...
  private handleJump(): void {
    ...
    if (this.canJump && Phaser.Input.Keyboard.JustDown(this.jumpKey)) {
      ...
      this.canJump = false;
      // Relative squish — keeps the tween proportional to whatever scale the player has
      this.tweens.add({
        targets: this.player,
