...
// ─── Ground constants ──────────────────────────────────────────────────────
//
// GROUND_Y is the Y coordinate of the TOP surface the player walks on.
// The player sprite has origin (0.5, 1) — feet at its Y position.
// So spawn at GROUND_Y and the player stands exactly on the ground line.
//
const GROUND_Y = 420;   // top of the walkable surface (px from top of screen)
const FLOOR_H  = 120;   // thickness of the invisible static platform below that

// ─── Jump config (single source of truth) ──────────────────────────────────
const JUMP_VY     = -520;   // upward impulse — negative = up
const GRAVITY_Y   = 800;    // world gravity set here in create(), not in main.ts
...
export class Chapter1Scene extends Phaser.Scene {
  ...
  private deerX = 980; private deerY = GROUND_Y - 30;
  ...
  // canJump is only reset by the collider callback — never by a timer
  ...
  create(): void {
    ...
    // ── FIX 1: Set gravity here (main.ts has gravity: {y:0} for top-down
    //   game — we override it just for this platformer scene)
    ...
    // ── FIX 2: Spawn player with feet ON the ground surface
    // Player entity should have setOrigin(0.5, 1) so that player.y = feet.
    // We spawn at GROUND_Y so feet touch the ground tile top.
    this.player = new Player(this, 80, GROUND_Y, gender);
    ...
    // ── FIX 3: canJump is set TRUE only when the collider fires —
    //   this is the canonical "is on ground" signal.
  ...
  private buildWorld(): void {
    ...
    // ── FIX 4: Single background — no mirrored/flipped duplicate ─────────
    // The background is drawn once, filling the full scene rectangle.
    // setScrollFactor(0) makes it a fixed parallax layer (moves slower than world).
    if (this.textures.exists('bg-ch1')) {
      // Real art: tile horizontally across the full map width
      this.add.tileSprite(MAP_W_CH1 / 2, MAP_H / 2, MAP_W_CH1, MAP_H, 'bg-ch1')
        .setDepth(DEPTH_BG)
        .setOrigin(0.5, 0.5);
      // NO second add.image / tileSprite below this line for the background.
    } else {
      // Procedural forest scene — covers full screen height in one pass
      this.buildProceduralBackground();
    }

    // ── Subtle shadow band at ground level (not a second background) ──────
    const depthGrad = this.add.graphics().setDepth(DEPTH_BG + 0.5);
    depthGrad.fillStyle(0x000000, 0.20);
    depthGrad.fillRect(0, GROUND_Y, MAP_W_CH1, FLOOR_H);

    // ── FIX 5: Ground platform sits at GROUND_Y ────────────────────────
    // Top of the physics box = GROUND_Y (centre Y + FLOOR_H/2 = GROUND_Y + FLOOR_H/2)
  ...
  // ── Procedural background — full scene, no mirroring ────────────────────
  private buildProceduralBackground(): void {
    const bg = this.add.graphics().setDepth(DEPTH_BG);

    // Sky gradient fills the FULL map height from top to bottom.
    // We use several bands rather than two halves — this avoids any seam
    // and removes the need for a flipped duplicate.
    const skyBands = [
      { y: 0,                    h: MAP_H * 0.12, color: 0x1a4a22 },
      { y: MAP_H * 0.12,         h: MAP_H * 0.15, color: 0x2e6b3a },
      { y: MAP_H * 0.27,         h: MAP_H * 0.15, color: 0x3a8a4a },
      { y: MAP_H * 0.42,         h: MAP_H * 0.16, color: 0x4aaa5a },
      { y: MAP_H * 0.58,         h: MAP_H * 0.14, color: 0x3a7a30 }, // ground zone begins
      { y: MAP_H * 0.72,         h: MAP_H * 0.28, color: 0x2d5a1b }, // under-ground dark fill
    ];
    for (const b of skyBands) {
      bg.fillStyle(b.color);
      bg.fillRect(0, b.y, MAP_W_CH1, b.h + 2); // +2 avoids 1px gaps at band edges
    }

    // Canopy tree silhouettes
    ...
    // Ground grass strip at GROUND_Y (visual — matches the physics platform)
  ...
  //   • body.setGravityY() was being called in both main.ts (world gravity=0)
  //     AND in create() (gravity=800), sometimes effectively doubling it.
  //   • canJump was being reset every frame from body.blocked.down, which
  //     can flicker to false the very frame after JustDown fires, so the
  //     velocity was immediately zeroed by a second code path.
  //
  // Fix:
  //   1. World gravity set ONCE in create() above.
  //   2. canJump = true  → only from the Arcade collider callback (reliable).
  //   3. canJump = false → only after we apply the jump velocity.
  //   4. We NEVER touch velocityY anywhere except the single setVelocityY call.
  //
  private handleJump(): void {
    const body = this.playerBody;

    // Allow jumping again if we land (collider fires this too, belt-and-braces)
    if (body.blocked.down) this.canJump = true;

    if (this.canJump && Phaser.Input.Keyboard.JustDown(this.jumpKey)) {
      body.setVelocityY(JUMP_VY);   // fire-and-forget — gravity does the rest
      this.canJump = false;
